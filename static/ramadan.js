...
async function loadTodayData() {
    ...
    try {
        const res = await fetch("/api/ramadan/today");
...
async function loadMonthData() {
    ...
    try {
        const res = await fetch(`/api/ramadan/month?lat=${lat}&lon=${lon}`);
...
async function loadDistrictData() {
    ...
    try {
        const res = await fetch(`/api/ramadan/province/${encodeURIComponent(province)}`);
        const data = await res.json();
