...
async function fetchPositions() {
    ...
    const res = await fetch(`${API_BASE}/api/positions?lat=${lat}&lon=${lon}`);
    return res.json();
}

async function fetchAllProvinces() {
    const res = await fetch(`${API_BASE}/api/all-provinces-positions`);
    return res.json();
}

async function fetchProvinceDetails(province) {
    const res = await fetch(`${API_BASE}/api/province-details/${encodeURIComponent(province)}`);
    return res.json();
}
...
async function loadDistricts(province) {
    const res = await fetch(`${API_BASE}/api/districts/${encodeURIComponent(province)}`);
